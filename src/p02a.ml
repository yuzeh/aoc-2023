...
let is_hand_possible bag = max_red >= bag.(0)  && max_green >= bag.(1) && max_blue >= bag.(2)
let parse_hand str =
  ...
  let rec agg handle score_accumulator =
    try
      ...
      let game_number =
        ...
        int_of_string (String.sub game_desc 5 (line_len - 5)) in
      let is_game_possible = (List.nth line_pair 1)
        |> String.trim
        |> String.split_on_char ';'
        |> List.map parse_hand 
        |> List.map is_hand_possible
        |> List.for_all (fun x -> x) in
      if is_game_possible
      then agg handle (score_accumulator + game_number)
