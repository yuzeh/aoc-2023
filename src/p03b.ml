...
let find_gear_ratios char_mat num_adj_mat =
  Array.mapi
    (fun i row ->
      Array.mapi
        (fun j c ->
          if c = '*' && List.length num_adj_mat.(i).(j) = 2 then
            prod num_adj_mat.(i).(j) :: []
          else [])
        row
...
let main filename =
  ...
  let gear_ratios = find_gear_ratios char_mat num_adj_mat in
  let gear_ratios_sum = List.fold_left ( + ) 0 gear_ratios in

  close_in ic;
  print_endline (string_of_int gear_ratios_sum)
